...
export async function getTemplates(templateName: string, title?: string) {
  ...
  // exclude    .tool
  filteredFiles = filteredFiles.filter((x) => x.path.startsWith('.tool') == false);

  // check for ignore file in .tool folder
  const templateIgnoreFile = files.find((x) => x.path.startsWith('.tool') && x.name == 'ignore');
...
`;
  let userMessage = ``;
  const templatePromptFile = files.filter((x) => x.path.startsWith('.tool')).find((x) => x.name == 'prompt');
